...
if __name__ == '__main__':
    ...
    with open(constants.RESULTS_FILE_PATH + 'PVs_' + file_name,'w') as pvs_file:
        ...
        for problem_id, subjects in problem_data.iteritems():
            ...
            for subj_id, subject in subjects.iteritems():
                ...
                print 'sub {}  pv {} '.format(subj_id, np.std(subject.get_outcomes()))
                pvs_file.write(str(problem_id) + ',')  # Problem ID
                pvs_file.write(str(subj_id) + ',')  # Subject ID
                pvs_file.write(str(np.std(subject.get_outcomes())) + '\n')  # PV
